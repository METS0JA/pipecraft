...
#load files
inputs = sort(list.files(pattern = fileFormat, full.names = TRUE))

#sample names
# sample_names = sapply(strsplit(basename(inputs), samp_ID), `[`, 1)
# print(sample_names)

#dereplicate
dereps = derepFastq(inputs)
#remove chimeras
a = removeBimeraDenovo(dereps)
#out to fasta
uniquesToFasta(a$F3D0.fastq, fout = "F3D0.uniquesToFasta.fasta")

#rereplicate fasta
vsearch --rereplicate F3D0.uniquesToFasta.fasta --fasta_width 0 --output F3D0.uniquesToFasta.rerep.fasta -relabel keep
